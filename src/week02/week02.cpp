...
namespace annzy::week02 {
    ...
    // std::non_of 사용으로도 풀 수 있음 (lambda 안에서 직접 주소 사용 x / prev, next 사용을
    // 권장한다) vector<bool>은 되도록 쓰지 말 것
    ...
    // 16. Snail Race
    // 모리스와 스티브가 3마리의 달팽이를 가지고 달팽이 경주를 하는데, 모리스가 최소 2번 이상
    // 승리하는지 They each have 3, a slow (s), medium (m) and fast (written [Maurice, Steve]) Round
    // 1: [s, f] Sacrifice his slowest snail against Steve's fastest. Round 2: [m, s] Use his middle
    // snail against Steve's slowest. Round 3: [f, m] Use his fastest snail against Steve's middle.
    // The function inputs:
    //
    ...
    auto spelling(std::string const& src) -> std::vector<std::string> {
        ...
        std::ranges::generate(result,
                              [&, length = 0]() mutable { return src.substr(0, ++length); });
        return result;
    }
    ...
    auto factorize(int n) -> std::vector<int> {
        ...
        // C++에서 시퀀스 표현: 가장 기본적으로는 배열(array)이나 벡터(vector) 등의 연속된 메모리
        // 공간에 요소들을 저장하는 데이터 구조가 있고, 그 외에도 리스트(list), 맵(map), 집합(set)
        // 등도 일정한 순서를 가지는 요소들의 집합으로 시퀀스로 간주
