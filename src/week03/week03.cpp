...
namespace annzy::week03 {
    ...
    auto identicalFilter(std::vector<std::string> const& arr) -> std::vector<std::string> {
        ...
        std::copy_if(
            arr.begin(), arr.end(), std::back_inserter(result), [](const std::string& str) {
    ...
    // std::transform_reduce 활용하면 더 간결한 코드
    ...
    auto countUnique(std::string const& str1, std::string const& str2) -> int {
        // char 타입의 값들을 저장하는 해시셋(HashSet)으로 str1의 모든 문자들로 초기화되어
        // str1에 있는 각 문자가 중복 없이 저장
        std::unordered_set<char> uniqueChars(str1.begin(), str1.end());
        // str2에 있는 모든 문자들이 중복 없이 uniqueChars에 추가
    ...
    auto accum(std::string const& s) -> std::string {
        ...
        int count = 0;
        for (char c : s) {
            // 첫 번째 문자가 아니면 -를 result에 추가
            if (count > 0) {
                result += '-';
            }
            result += std::toupper(c);  // 대문자로 변환
            // count 개수만큼 c를 소문자로 변환한 문자열을 result에 추가
            result += std::string(count, std::tolower(c));
            ++count;
        }
