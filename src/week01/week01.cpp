                   ...
                   const std::vector<std::string> &word2) -> std::vector<int>
{
    ...
    // 일반적으로 std::transform을 사용할 때는 변환 대상 범위와 결과를 저장할 범위는 반드시 동일한 크기
    // std::transform(범위의 시작, 범위의 끝, 비교범위 시작, 결과를 저장할 시작점, 람다식)
    std::transform(word1.begin(), word1.end(), word2.begin(), result.begin(),
                   [](const std::string &w1, const std::string &w2) { // []는 C++에서 람다(lambda) 함수를 정의할 때 사용
                       return (w1 == w2) ? 1 : -1;
                   });
    return result;
}
...
auto existsHigher(const std::vector<int> &nums, int standardNum) -> bool
{
    ...
    return std::any_of(nums.begin(), nums.end(), [standardNum](int num)
